#!/usr/bin/env python3
"""
Bootstrap Virtual Environments for Personal Command-Line Python Tools (v_combined_update)
...
def create_bash_wrapper(
    ...
    """Generates and writes the bash wrapper script, using shlex.quote for safety."""
    ...
    python_exe_name = 'Scripts/python.exe' if platform.system() == "Windows" else 'bin/python'
    venv_python_exe = (abs_venv_path / python_exe_name).resolve()

    # Use shlex.quote for all path variables
...
PYTHON_EXE={shlex.quote(str(venv_python_exe))}
...
if [ ! -x "$PYTHON_EXE" ]; then
    echo "Error: Python executable not found or not executable at $PYTHON_EXE" >&2
    exit 1
fi
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bootstrap Virtual Environments for Personal Command-Line Tools (v_combined_update)",
